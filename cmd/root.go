...
func addSubCommands() {
	RootCmd.AddCommand(
		// from 'import daily-notes/cmd/<subcommand:package>'
		// <package>.InitSubCommands(c),
		add.InitSubCommands(c),
		get.InitSubCommands(c),
		test.InitSubCommands(c),
	)
}
...
func initConfig() {
	if cfgFile != "" {
		...
		workDir := fmt.Sprintf("%s/.config/daily-notes", home)
		if _, err := os.Stat(workDir); err != nil {
			if os.IsNotExist(err) {
				mkerr := os.MkdirAll(workDir, os.ModePerm)
				if mkerr != nil {
					logrus.Fatal("Error creating ~/.config/daily-notes directory", mkerr)
				}
			}
		}
		if stat, err := os.Stat(workDir); err == nil && stat.IsDir() {
			...
			logrus.Info("The ~/.config/daily-notes path is a file and not a directory, please remove the 'daily-notes' file.")
			os.Exit(1)
		}
	}
